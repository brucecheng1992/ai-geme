...
export type ArtProviderLivePreflightResult = {
  ...
  credential: {
    status: 'referenced' | 'available_without_ref' | 'missing' | 'not_required';
...
export function resolveArtProviderLivePreflight(input: ArtProviderLivePreflightInput = {}): ArtProviderLivePreflightResult {
  ...
  if (requestedProvider === undefined) {
    return buildPreflightResult({
      ...
      invalidFields: ['requestedProvider'],
...
export function readArtProviderLivePreflightFromEnv(env: Record<string, string | undefined>): ArtProviderLivePreflightInput {
  ...
  const allowNetwork = parseBooleanEnv(env.AI_GEME_ALLOW_LIVE_ART_NETWORK);
  const credentialAvailable =
    hasNonEmptyString(env.AI_GEME_ART_PROVIDER_CREDENTIAL_REF) || parseBooleanEnv(env.AI_GEME_ART_PROVIDER_CREDENTIAL_AVAILABLE);
  const costAcknowledged = parseBooleanEnv(env.AI_GEME_ART_PROVIDER_COST_ACKNOWLEDGED);
  const budgetLimitCents = parseNumberEnv(env.AI_GEME_ART_PROVIDER_BUDGET_LIMIT_CENTS);
...
function livePreflightBlockers(
  ...
  if (audit.credential.status === 'missing') blockers.push('art_provider_live_credentials_missing');
...
function auditStateForLive(input: ArtProviderLivePreflightInput): LiveAuditState {
  ...
  return {
    ...
    credential: {
      status: credentialRefPresent ? 'referenced' : credentialAvailable ? 'available_without_ref' : 'missing',
