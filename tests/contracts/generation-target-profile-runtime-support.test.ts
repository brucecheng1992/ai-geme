...
describe('Step 37 target profile runtime support overlay', () => {
  ...
  it('keeps UI failure restart unverified when generic restart evidence lacks failure-screen reset state', () => {
    const dependencyEvidence: CapabilityRuntimeObservedProbeEvidence[] = [
      {
        capabilityId: healthCapabilityId,
        probeId: HEALTH_PLAYER_HEALTH_POINTS_REQUIRED_PROBE_ID,
    ...
    const genericRestartQaReport = buildSingleCapabilityQaReport({
      capabilityId: uiFailureRestartCapabilityId,
      packageContract: createUiFailureRestartPackageContract(),
    ...
    const observedFailureRestartQaReport = buildSingleCapabilityQaReport({
      capabilityId: uiFailureRestartCapabilityId,
      packageContract: createUiFailureRestartPackageContract(),
      dependencyPackages: [createHealthPlayerHealthPointsPackageContract(), createRulesRetryCountPackageContract()],
      additionalObserved: dependencyEvidence,
      ...
      stateFields: failureRestartStateFields()
    });
    const genericRestartReport = buildGenerationTargetProfileRuntimeSupportReport({
      projectId: 'proj_20260626_target_runtime_support',
      runId: 'run_20260626_ui_failure_restart_generic_restart',
      capabilityQaReport: genericRestartQaReport
    });
    const observedFailureRestartReport = buildGenerationTargetProfileRuntimeSupportReport({
      projectId: 'proj_20260626_target_runtime_support',
      runId: 'run_20260626_ui_failure_restart_observed_state',
      capabilityQaReport: observedFailureRestartQaReport
    });
    const genericRestartState = genericRestartReport.capabilities.find((entry) => entry.capabilityId === uiFailureRestartCapabilityId);
    const observedFailureRestartState = observedFailureRestartReport.capabilities.find((entry) => entry.capabilityId === uiFailureRestartCapabilityId);

    expect(genericRestartQaReport.requiredResults.find((entry) => entry.probeId === HEALTH_PLAYER_HEALTH_POINTS_REQUIRED_PROBE_ID)).toMatchObject({
      status: 'passed'
    });
    expect(genericRestartQaReport.requiredResults.find((entry) => entry.probeId === RULES_RETRY_COUNT_REQUIRED_PROBE_ID)).toMatchObject({
      status: 'passed'
    });
    expect(genericRestartQaReport.requiredResults.find((entry) => entry.probeId === UI_FAILURE_RESTART_REQUIRED_PROBE_ID)).toMatchObject({
      status: 'failed',
      assertionResults: expect.arrayContaining([
        expect.objectContaining({
          assertionId: `${UI_FAILURE_RESTART_REQUIRED_PROBE_ID}.assertion.failure_restart_verified`,
          status: 'failed',
    ...
    expect(genericRestartState).toMatchObject({
      runtimeVerified: false,
      observedCompleteSupported: false,
      verifiedRequiredProbeIds: [],
      missingRequiredProbeIds: [UI_FAILURE_RESTART_REQUIRED_PROBE_ID],
      observedEvidenceDimensions: { qa_observed: false }
    });
    expect(observedFailureRestartState).toMatchObject({
      runtimeVerified: true,
      staticCompleteSupported: false,
      observedCompleteSupported: true,
  ...
  it('keeps boss health HUD unverified when boss lifecycle exists but HUD binding fields are missing', () => {
    const dependencyEvidence: CapabilityRuntimeObservedProbeEvidence[] = [
      {
        capabilityId: enemyBossLifecycleCapabilityId,
        probeId: ENEMY_BOSS_LIFECYCLE_REQUIRED_PROBE_ID,
    ...
    const lifecycleOnlyQaReport = buildSingleCapabilityQaReport({
      capabilityId: uiHudBossHealthCapabilityId,
      packageContract: createUiHudBossHealthPackageContract(),
      dependencyPackages: [createEnemyBossLifecyclePackageContract()],
      additionalObserved: dependencyEvidence,
      eventType: ENEMY_BOSS_LIFECYCLE_EVENT_TYPE,
      eventTypes: [ENEMY_BOSS_LIFECYCLE_EVENT_TYPE],
      probeId: UI_HUD_BOSS_HEALTH_REQUIRED_PROBE_ID,
    ...
    const observedHudQaReport = buildSingleCapabilityQaReport({
      ...
      stateFields: hudBossHealthStateFields()
    });
    const lifecycleOnlyReport = buildGenerationTargetProfileRuntimeSupportReport({
      projectId: 'proj_20260626_target_runtime_support',
      runId: 'run_20260626_ui_hud_boss_health_lifecycle_only',
      capabilityQaReport: lifecycleOnlyQaReport
    });
    const observedHudReport = buildGenerationTargetProfileRuntimeSupportReport({
      projectId: 'proj_20260626_target_runtime_support',
      runId: 'run_20260626_ui_hud_boss_health_observed_state',
      capabilityQaReport: observedHudQaReport
    });
    const lifecycleOnlyState = lifecycleOnlyReport.capabilities.find((entry) => entry.capabilityId === uiHudBossHealthCapabilityId);
    const observedHudState = observedHudReport.capabilities.find((entry) => entry.capabilityId === uiHudBossHealthCapabilityId);

    expect(lifecycleOnlyQaReport.requiredResults.find((entry) => entry.probeId === ENEMY_BOSS_LIFECYCLE_REQUIRED_PROBE_ID)).toMatchObject({
      status: 'passed'
    });
    expect(lifecycleOnlyQaReport.requiredResults.find((entry) => entry.probeId === UI_HUD_BOSS_HEALTH_REQUIRED_PROBE_ID)).toMatchObject({
      status: 'failed',
      assertionResults: expect.arrayContaining([
        expect.objectContaining({
          assertionId: `${UI_HUD_BOSS_HEALTH_REQUIRED_PROBE_ID}.assertion.boss_health_hud_verified`,
          status: 'failed',
    ...
    expect(lifecycleOnlyState).toMatchObject({
      runtimeVerified: false,
      observedCompleteSupported: false,
      verifiedRequiredProbeIds: [],
      missingRequiredProbeIds: [UI_HUD_BOSS_HEALTH_REQUIRED_PROBE_ID],
      observedEvidenceDimensions: { qa_observed: false }
    });
    expect(observedHudState).toMatchObject({
      ...
      staticCompleteSupported: false,
      observedCompleteSupported: true,
      requiredProbeIds: [UI_HUD_BOSS_HEALTH_REQUIRED_PROBE_ID],
      verifiedRequiredProbeIds: [UI_HUD_BOSS_HEALTH_REQUIRED_PROBE_ID],
      missingRequiredProbeIds: [],
      staticEvidenceDimensions: { qa_observed: false },
      observedEvidenceDimensions: { qa_observed: true }
    });
  ...
  it('keeps current weapon HUD unverified when weapon events exist but HUD binding fields are missing', () => {
    const dependencyEvidence: CapabilityRuntimeObservedProbeEvidence[] = [
      {
        capabilityId: defaultWeaponCapabilityId,
        probeId: DEFAULT_STRAIGHT_SINGLE_WEAPON_REQUIRED_PROBE_ID,
    ...
    const weaponOnlyQaReport = buildSingleCapabilityQaReport({
    ...
    const weaponOnlyReport = buildGenerationTargetProfileRuntimeSupportReport({
      projectId: 'proj_20260626_target_runtime_support',
      runId: 'run_20260626_ui_hud_current_weapon_weapon_only',
      capabilityQaReport: weaponOnlyQaReport
    });
    const observedHudReport = buildGenerationTargetProfileRuntimeSupportReport({
      projectId: 'proj_20260626_target_runtime_support',
      runId: 'run_20260626_ui_hud_current_weapon_observed_state',
      capabilityQaReport: observedHudQaReport
    });
    const weaponOnlyState = weaponOnlyReport.capabilities.find((entry) => entry.capabilityId === uiHudCurrentWeaponCapabilityId);
    const observedHudState = observedHudReport.capabilities.find((entry) => entry.capabilityId === uiHudCurrentWeaponCapabilityId);

    expect(weaponOnlyQaReport.requiredResults.find((entry) => entry.probeId === DEFAULT_STRAIGHT_SINGLE_WEAPON_REQUIRED_PROBE_ID)).toMatchObject({
      status: 'passed'
    });
    expect(weaponOnlyQaReport.requiredResults.find((entry) => entry.probeId === UI_HUD_CURRENT_WEAPON_REQUIRED_PROBE_ID)).toMatchObject({
      status: 'failed',
      assertionResults: expect.arrayContaining([
        expect.objectContaining({
          assertionId: `${UI_HUD_CURRENT_WEAPON_REQUIRED_PROBE_ID}.assertion.current_weapon_hud_verified`,
          status: 'failed',
    ...
    expect(weaponOnlyState).toMatchObject({
      runtimeVerified: false,
      observedCompleteSupported: false,
      verifiedRequiredProbeIds: [],
      missingRequiredProbeIds: [UI_HUD_CURRENT_WEAPON_REQUIRED_PROBE_ID],
      observedEvidenceDimensions: { qa_observed: false }
    });
    expect(observedHudState).toMatchObject({
      runtimeVerified: true,
      staticCompleteSupported: false,
      observedCompleteSupported: true,
      requiredProbeIds: [UI_HUD_CURRENT_WEAPON_REQUIRED_PROBE_ID],
...
function failureRestartStateFields(): Record<string, unknown> {
  return {
...
function hudBossHealthStateFields(): Record<string, unknown> {
  return {
...
function hudCurrentWeaponStateFields(): Record<string, unknown> {
  return {
