...
describe('Step37 remaining complete-supported inventory driver', () => {
  ...
  it('derives the current Stage 4 inventory from the real support summary and explicit committed closure history', () => {
    ...
    expect(report.registeredCapabilityCount).toBe(47);
    expect(report.staticCompleteSupportedCount).toBe(0);
    expect(report.stateCounts.unsupported_unregistered).toBe(12);
    expect(report.committedClosedCapabilityCount).toBe(46);
    ...
    expect(report.nextCheckpoint?.checkpoint_id).toBe('stage4.spawn_explicit_declarations_v1.complete_supported_package_slice');
    expect(report.nextCheckpoint?.next_atomic_step).toBe(
      'Stage 4 spawn.explicit_declarations.v1 complete-supported package slice implementation atomic step'
    );
