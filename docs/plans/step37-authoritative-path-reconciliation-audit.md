...
> - 当前状态：Stage 4 combat.airborne_fire package-owned QA slice audit passed Oracle; checkpoint pending
...
- 工作区核对：Stage 4 `health.player_health_points.v1` package-owned QA slice checkpoint is `71e83e0c`; current uncommitted diff is docs-only for `combat.airborne_fire.v1` audit and passed Oracle, awaiting checkpoint commit.
...
Stage 4 combat.airborne_fire package-owned QA slice audit: ORACLE_PASSED_AWAITING_COMMIT
Stage 4 combat.airborne_fire package-owned QA slice implementation: NOT_ENTERED
Stage 4 Exit gate: NOT_MET
```

下一步只允许提交 Stage 4 `combat.airborne_fire.v1` package-owned QA slice audit checkpoint；不得跳过 audit checkpoint 直接实现，不得进入 Stage 5，不得把预期 `8/59` runtime-observed support 等同于 Stage 4 exit。
