...
> - 当前状态：Stage 4 combat.airborne_fire package-owned QA slice implementation passed Oracle; checkpoint pending
...
- 工作区核对：Stage 4 `combat.airborne_fire.v1` package-owned QA slice audit checkpoint is `217c67a8`; current uncommitted diff implements only this airborne-fire slice and passed Oracle, awaiting checkpoint commit.
...
Stage 4 combat.airborne_fire package-owned QA slice implementation: ORACLE_PASSED_AWAITING_COMMIT
Stage 4 Exit gate: NOT_MET
```

下一步只允许提交 Stage 4 `combat.airborne_fire.v1` package-owned QA slice checkpoint；不得进入下一 Stage 4 slice audit，不得进入 Stage 5，不得把 `8/59` runtime-observed support 等同于 Stage 4 exit。
